...
Travian.Game.Map.LowRes.Container = (function () {
  ...
  var f = function (h, i, j) {
    if (!i.rightClick && h.isEventsEnabled()) {
      if (h.tileDisplayInformation.type === 'dialog') {
        h.disableEvents();
        new Travian.Dialog.Ajax(
          Travian.Helpers.deepmergeObject(
            {},
            h.tileDisplayInformation.optionsDialog,
            {
              context: 'map',
              data: j,
              onOpen: function (l, k) {
                jQuery(k)
                  ...
                  .on('click', function (n) {
                    n.preventDefault();
                    var m = Travian.parseURL(n.target.href);
                    h.moveTo({
                      x: parseInt(m.searchObject.x || '0'),
                      y: parseInt(m.searchObject.y || '0'),
                    });
                    l.close();
                    return false;
                  });
        ...
        Travian.popup(
          Travian.Helpers.substitute(
            h.tileDisplayInformation.optionsPopup.url,
            j
          ),
          h.tileDisplayInformation.optionsPopup.windowOptions
        );
      }
    }
  };
  var e = function (h) {
    if (h.match(/x\{([-0123456789]+)\}\sy\{([-0123456789]+)\}/gi) == null) {
      return;
    }
    return { x: RegExp.$1, y: RegExp.$2 };
  };
  var g = function (h) {
    h.elements.containerData.find('.tile').on({
      mouseup: function (i) {
        f(h, i, e(i.target.className));
      },
    });
  };
  var c = function (h) {
    ...
    d = setTimeout(function () {
      ...
      b = Travian.ajax({
        ...
        data: {
          cmd: 'mapLowRes',
          data: {
            x: h.mapInitialPosition.x,
            y: h.mapInitialPosition.y,
            width: h.containerSize.width,
            height: h.containerSize.height,
          },
        },
        onSuccess: function (i) {
          if (h.elements.fullScreen) {
            h.elements.fullScreen.attr(
              'href',
              Travian.Helpers.substitute(h.fullScreenUrl, {
                fullScreen: h.fullScreen ? 0 : 1,
                x: h.mapInitialPosition.x,
                y: h.mapInitialPosition.y,
              })
            );
          }
          h.elements.containerData.html(i.html);
          h.container.show();
          g(h);
          h.update();
          Travian.Tip.refresh();
  ...
  var a = function (h) {
    this.classType = 'Travian.Game.Map.LowRes.Container';
    this.elements = { tiles: [] };
    ...
    this.move = function (i) {
      this.mapInitialPosition.x += i.x;
      this.mapInitialPosition.y += i.y;
      c(this);
      return this;
    };
    ...
    this.render = function () {
      ...
      this.elements.fullScreen = this.container.find('#navigationFullScreen');
      this.elements.containerData = this.container.find('.mapContainerData');
      this.elements.positionEnterX = this.container.find(
        '#mapCoordEnter .coordinates.x'
      );
      this.elements.positionEnterY = this.container.find(
        '#mapCoordEnter .coordinates.y'
      );
      var j = this.container.is(':visible');
      if (!j) {
        this.container.show();
      }
      this.containerSize = {
      ...
      for (var i in this.options.Navigation) {
        if (this.options.Navigation.hasOwnProperty(i)) {
          k.navigations[i] = new Travian.Game.Map.LowRes.Navigation(
            this.options.Navigation[i],
            k
          );
        }
      }
      this.container
        .find('#mapCoordEnter button.green')
        .on('click', function (l) {
          l.stopPropagation();
          k.moveTo({
            x: k.elements.positionEnterX.val(),
            y: k.elements.positionEnterY.val(),
          });
          return false;
        });
      if (this.fullScreen) {
    ...
    this.update = function () {
      for (var i in this.navigations) {
        if (this.navigations.hasOwnProperty(i)) {
          this.navigations[i].update();
        }
      }
      return this;
    };
    Travian.Game.Map.Base.call(this, h);
    this.render();
  };
...
Travian.Game.Map.LowRes.Navigation = function (a, b) {
  this.classType = 'Travian.Game.Map.LowRes.Navigation';
  this.render = function () {
    var d = this;
    var c = {
      count: 0,
    ...
    this.element = jQuery(this.element).on('click', function (f) {
      f.stopPropagation();
      d.parentContainer.move(d.delta.normal);
      d.update();
      return false;
    });
    this.keyboard = this.keyboard.filter(function (e) {
      return e !== false;
    });
    this.keyboard = (function (f) {
      var e = {};
      f.forEach(function (g) {
        e[g] = { on: ['keydown', 'keyup'], periodical: 200, fn: 'move' };
        c.keys[g] = false;
      });
      return e;
    })(this.keyboard);
    jQuery(window).on({
      keydown: function (f) {
        if (!d.parentContainer.isEventsEnabled()) {
          return;
        }
        if (f.shiftKey) {
          c.shift = true;
        ...
        if (
          c.keys[f.keyCode] === false &&
          f.target.nodeName.toLowerCase() !== 'input'
        ) {
          if (d.keyboard[f.keyCode].on.indexOf('keydown') === -1) {
            return;
          }
          c.count++;
          c.keys[f.keyCode] = true;
          f.stopPropagation();
          if (!c.fnTimer) {
            c.fn = function () {
              for (var g in c.keys) {
                if (c.keys.hasOwnProperty(g)) {
                  var i = c.keys[g];
                  if (i) {
                    if (!d.keyboard[g]) {
                      return;
                    }
                    var k = d.keyboard[g].fn;
                    if (k === false || !d.parentContainer[k]) {
                      return;
                    }
                    var h = '';
                    h = 'normal';
                    ...
                    d.parentContainer[k](d.delta[h]);
                  }
                }
              }
            };
            if (d.keyboard[f.keyCode].periodical === 0) {
              c.fn();
            } else {
              if (d.keyboard[f.keyCode].periodical > 0) {
                c.fn();
                c.fnTimer = setInterval(c.fn, d.keyboard[f.keyCode].periodical);
              }
            }
          }
        }
      ...
      keyup: function (f) {
        ...
        if (c.keys[f.keyCode] === true) {
          if (d.keyboard[f.keyCode].on.indexOf('keyup') === -1) {
            return;
          }
          c.count--;
          c.keys[f.keyCode] = false;
          f.stopPropagation();
          if (c.count === 0 && c.fnTimer) {
            clearInterval(c.fnTimer);
            c.fnTimer = null;
          }
        }
      },
  ...
  this.update = function () {
    if (this.element && this.href) {
      var c = Object.assign({}, this.parentContainer.mapInitialPosition);
      c.x += this.delta.normal.x;
      c.y += this.delta.normal.y;
      this.element.attr('href', Travian.Helpers.substitute(this.href, c));
    }
    return this;
  };
...
Travian.Game.Map.LowRes.Navigation.prototype = Object.create(
  Travian.Game.Map.Base.prototype
);
Travian.Game.Map.LowRes.Navigation.constructor =
  Travian.Game.Map.LowRes.Navigation;
Travian.Game.Map.LowRes.Options = {};
Travian.Game.Map.LowRes.Options.Default = {
  request: { delayTime: 10, url: 'ajax.php' },
