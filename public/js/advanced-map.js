class AdvancedMapManager {
  ...
  setupEventListeners() {
    ...
    // Livewire events
    window.addEventListener('mapUpdated', (e) =>
      this.handleMapUpdate(e.detail)
    );
    window.addEventListener('villageSelected', (e) =>
      this.handleVillageSelection(e.detail)
    );
    window.addEventListener('worldChanged', (e) =>
      this.handleWorldChange(e.detail)
    );
    window.addEventListener('mapModeChanged', (e) =>
      this.handleMapModeChange(e.detail)
    );
  }
  ...
  selectVillage(village) {
    ...
    window.dispatchEvent(
      new CustomEvent('villageSelected', {
  ...
  updateOverlay() {
    ...
    if (mapMode)
      mapMode.textContent = this.realWorldMode
        ? 'Real World'
        : 'Game Coordinates';
  ...
  updateLivewireData() {
    ...
    if (window.Livewire) {
      ...
      window.Livewire.find(
        document.querySelector('[wire\\:id]').getAttribute('wire:id')
      ).set('realWorldMode', this.realWorldMode);
    }
  }
  ...
  drawGrid() {
    ...
    const endX =
      startX + Math.ceil(this.canvas.width / this.scale / gridSize) * gridSize;
    const endY =
      startY + Math.ceil(this.canvas.height / this.scale / gridSize) * gridSize;
