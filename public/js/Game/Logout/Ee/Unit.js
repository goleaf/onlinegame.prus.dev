  ...
  unitToAttack: {
    ...
    set: function (unitToAttack) {
      ...
      // neue einheit angreifen
        ...
        this._walkInterval = this.walk.periodical(
          Ee.Math.random(
            Ee.Balancing.fight.walkTimeInterval.min,
            Ee.Balancing.fight.walkTimeInterval.max
          ),
          this
        );
        this._attackingInterval = this.attack.periodical(
          Ee.Math.random(
            Ee.Balancing.fight.attackTimeInterval.min,
            Ee.Balancing.fight.attackTimeInterval.max
          ),
          this
        );

        Ee.log(
          this.type + ' ' + this.id + ' is attacking ' + this._unitToAttack.type
        );
      }
    },
  },
...
Ee.Unit.prototype.distanceToUnitToAttack = function () {
  ...
  if (positionThisUnit.x == positionOtherUnit.x) {
    ...
    dist = Math.abs(
      positionThisUnit.y +
        (sign * this.size.y) / 2 -
        (positionOtherUnit.y + (-1 * sign * this.unitToAttack.size.y) / 2)
    );
  }
  ...
  // zum rand der einheiten bestimmen
    ...
    var n =
      positionOtherUnit.x -
      (positionThisUnit.x * positionOtherUnit.y) / positionThisUnit.y;
    var m = (positionOtherUnit.x - n) / positionOtherUnit.y;

    var thisUnitX = positionThisUnit.x + (sign * this.size.x) / 2;
    var otherUnitX =
      positionOtherUnit.x + (-1 * sign * this.unitToAttack.size.x) / 2;
    ...
    dist = Math.sqrt(
      Math.pow(thisUnitX - otherUnitX, 2) +
        Math.pow(m * thisUnitX + n - (m * otherUnitX + n), 2)
    );
  }
...
Ee.Unit.prototype.walk = function () {
  ...
  var speed =
    this.speed * Ee.Math.sign(positionOtherUnit.x, positionThisUnit.x);
  var m =
    (positionOtherUnit.y - positionThisUnit.y) /
    (positionOtherUnit.x - positionThisUnit.x);
  var n = positionOtherUnit.y - m * positionOtherUnit.x;

  var moveByY =
    positionThisUnit.x - this.size.x / 2 <= positionOtherUnit.x &&
    positionOtherUnit.x <= positionThisUnit.x + this.size.x / 2;
