class AdvancedMapManager {
  ...
  updateStatistics() {
    ...
    if (mapData) {
      ...
      if (totalVillages)
        totalVillages.textContent = mapData.dataset.totalVillages || 0;
      if (myVillages) myVillages.textContent = mapData.dataset.myVillages || 0;
      if (allianceVillages)
        allianceVillages.textContent = mapData.dataset.allianceVillages || 0;
      if (enemyVillages)
        enemyVillages.textContent = mapData.dataset.enemyVillages || 0;
    }
  }
  ...
  updateOverlay() {
    ...
    if (mapMode)
      mapMode.textContent = this.showRealWorld
