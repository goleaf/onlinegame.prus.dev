...
(function (d) {
  ...
  i.prototype = {
    ...
    setOptions: function (b) {
      ...
      if (a.lookup) {
        this.isLocal = true;
        if (d.isArray(a.lookup)) a.lookup = { suggestions: a.lookup, data: [] };
      }
      d('#' + this.mainContainerId).css({ zIndex: a.zIndex });
      this.container.css({ maxHeight: a.maxHeight + 'px', width: a.width });
    },
    clearCache: function () {
      this.cachedResponse = [];
      this.badQueries = [];
    ...
    suggest: function () {
      if (this.suggestions.length === 0) this.hide();
        ...
        for (f = 0; f < a; f++) {
          ...
          c = d(
            (b.selectedIndex === f ? '<div class="selected"' : '<div') +
              ' title="' +
              c +
              '">' +
              e(c, this.data[f], g) +
