...
function updateBuildingProgress(buildingId, progress) {
    const progressElement = document.querySelector(`[data-building-id="${buildingId}"] .building-progress`);
    if (progressElement) {
...
function updateTrainingProgress(unitId, progress) {
    const progressElement = document.querySelector(`[data-unit-id="${unitId}"] .training-progress`);
    if (progressElement) {
        progressElement.style.width = progress + '%';
    }
}
...
function updateResourceDisplay(resource) {
    const resourceElement = document.querySelector(`[data-resource="${resource.type}"] .resource-amount`);
    if (resourceElement) {
...
function updateResourceProduction(production) {
    Object.keys(production).forEach(resource => {
        const productionElement = document.querySelector(`[data-resource="${resource}"] .resource-production`);
        if (productionElement) {
            productionElement.textContent = '+' + formatNumber(production[resource]) + '/h';
        }
    });
}

function updateVillageDisplay(village) {
    const villageElement = document.querySelector(`[data-village-id="${village.id}"]`);
    if (villageElement) {
        villageElement.querySelector('.village-population').textContent = formatNumber(village.population);
        villageElement.querySelector('.village-culture').textContent = formatNumber(village.culture_points);
    }
}
...
function showBattleReportModal(report) {
    ...
    modal.className = 'travian-modal fixed inset-0 z-50 flex items-center justify-center';
    modal.innerHTML = `
...
function showBattleReportNotification(report) {
    showNotification(`Battle report: ${report.result} at ${report.location}`, 'warning');
}
