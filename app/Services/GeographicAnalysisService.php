...
class GeographicAnalysisService
{
    ...
    {
        if ($density < 0.1) return 'Very Low';
    ...
    protected function categorizeDistances(array $distances): array
    {
        ...
        foreach ($distances as $distance) {
            if ($distance < 5) $categories['very_short']++;
            elseif ($distance < 20) $categories['short']++;
            elseif ($distance < 50) $categories['medium']++;
            elseif ($distance < 100) $categories['long']++;
            else $categories['very_long']++;
        }
    ...
    protected function analyzeBearings(array $bearings): array
    {
        ...
        foreach ($bearings as $bearing) {
            if ($bearing >= 337.5 || $bearing < 22.5) $directions['north']++;
            elseif ($bearing < 67.5) $directions['northeast']++;
            elseif ($bearing < 112.5) $directions['east']++;
            elseif ($bearing < 157.5) $directions['southeast']++;
            elseif ($bearing < 202.5) $directions['south']++;
            elseif ($bearing < 247.5) $directions['southwest']++;
            elseif ($bearing < 292.5) $directions['west']++;
            else $directions['northwest']++;
        }
