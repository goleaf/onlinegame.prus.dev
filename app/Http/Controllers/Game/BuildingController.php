...
class BuildingController extends CrudController
{
    ...
    public function villageBuildings(int $villageId): JsonResponse
    {
        try {
            ...
            $cacheKey = "village_buildings_{$villageId}_{$playerId}";

            $buildings = CachingUtil::remember($cacheKey, now()->addMinutes(10), function () use ($villageId, $playerId) {
                $village = Village::where('player_id', $playerId)
                    ->findOrFail($villageId);

                return Building::with(['buildingType'])
                    ->where('village_id', $villageId)
                    ->get();
            });

            LoggingUtil::info('Village buildings retrieved', [
                'user_id' => auth()->id(),
                'player_id' => $playerId,
            ...
            LoggingUtil::error('Error retrieving village buildings', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
                'village_id' => $villageId,
            ], 'building_system');
    ...
    public function show(int $id): JsonResponse
    {
        try {
            $playerId = Auth::user()->player->id;
            $cacheKey = "building_{$id}_{$playerId}";
            ...
            LoggingUtil::info('Building details retrieved', [
                'user_id' => auth()->id(),
                'player_id' => $playerId,
                'building_id' => $id,
            ], 'building_system');
            ...
            LoggingUtil::error('Error retrieving building details', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
                'building_id' => $id,
            ], 'building_system');
    ...
    public function upgrade(int $id): JsonResponse
    {
        try {
            ...
            CachingUtil::forget("building_{$id}_{$playerId}");
            CachingUtil::forget("village_buildings_{$village->id}_{$playerId}");

            DB::commit();

            LoggingUtil::info('Building upgrade started', [
                'user_id' => auth()->id(),
                'player_id' => $playerId,
                'building_id' => $id,
                'target_level' => $building->level + 1,
            ...
            DB::rollBack();
            LoggingUtil::error('Error starting building upgrade', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
                'building_id' => $id,
            ], 'building_system');
    ...
    public function cancelUpgrade(int $id): JsonResponse
    {
        try {
            ...
            LoggingUtil::info('Building upgrade cancelled', [
                'user_id' => auth()->id(),
                'player_id' => $playerId,
                'building_id' => $id,
            ...
            DB::rollBack();
            LoggingUtil::error('Error cancelling building upgrade', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
                'building_id' => $id,
            ], 'building_system');
    ...
    public function buildingQueue(int $villageId): JsonResponse
    {
        try {
            $playerId = Auth::user()->player->id;
            $cacheKey = "building_queue_{$villageId}_{$playerId}";

            $buildingQueue = CachingUtil::remember($cacheKey, now()->addMinutes(5), function () use ($villageId, $playerId) {
                $village = Village::where('player_id', $playerId)
                    ->findOrFail($villageId);
            ...
            LoggingUtil::info('Building queue retrieved', [
                'user_id' => auth()->id(),
                'player_id' => $playerId,
                'village_id' => $villageId,
                'queue_count' => $buildingQueue->count(),
            ], 'building_system');

            return $this->successResponse($buildingQueue, 'Building queue retrieved successfully.');
        } catch (\Exception $e) {
            LoggingUtil::error('Error retrieving building queue', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
                'village_id' => $villageId,
            ], 'building_system');
    ...
    {
        try {
            $cacheKey = 'building_types_all';

            $buildingTypes = CachingUtil::remember($cacheKey, now()->addHours(1), function () {
                return BuildingType::all();
            });

            LoggingUtil::info('Building types retrieved', [
                'user_id' => auth()->id(),
                'types_count' => $buildingTypes->count(),
            ], 'building_system');

            return $this->successResponse($buildingTypes, 'Building types retrieved successfully.');
        } catch (\Exception $e) {
            LoggingUtil::error('Error retrieving building types', [
                'error' => $e->getMessage(),
                'user_id' => auth()->id(),
            ], 'building_system');
